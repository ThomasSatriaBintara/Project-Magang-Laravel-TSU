...
@php
    ...
    if ($isAdmin) {
        $namaLengkap = "Teguh Susyanto";
        $identitasLabel = "NUPTK";
        $identitasValue = "0122334455";
    } elseif ($isDosen) {
        $namaLengkap = "Wawan Laksito";
        $identitasLabel = "NUPTK";
        $identitasValue = "061234567";
    } else {
        $namaLengkap = "Thomas Satria Bintara";
        $identitasLabel = "NIM";
        $identitasValue = "22430035";
    }
@endphp
...
<div class="max-w-6xl mx-auto pb-10">
    ...
    <div class="fade-up flex flex-wrap items-center gap-2 mb-8 bg-white p-2 rounded-3xl border border-gray-100 shadow-sm">
        ...
        @if(!$isAdmin)
        <button onclick="switchTab('extra')" id="btn-extra" class="fade-up tab-btn flex-1 flex items-center justify-center gap-3 px-6 py-4 rounded-2xl font-bold transition-all text-gray-500 hover:bg-gray-50 relative">
            @if($isDosen)
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="fade-up delay-100 bg-white border border-gray-200 rounded-[2.5rem] p-8 md:p-12 shadow-sm min-h-[500px]">
            ...
            <form action="#" class="space-y-8">
                ...
                <div class="fade-up delay-300 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" value="{{ $namaLengkap }}" disabled class="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 text-gray-400 cursor-not-allowed">
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-bold text-gray-600 ml-1">{{ $identitasLabel }}</label>
                        <input type="text" value="{{ $identitasValue }}" disabled class="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 text-gray-400 cursor-not-allowed">
                    </div>
                </div>
        ...
        <div id="tab-extra" class="tab-content hidden">
            @if($isDosen)
                <h3 class="fade-up text-2xl font-black text-gray-800 mb-2">Nomor WhatsApp</h3>
                <p class="fade-up delay-100 text-gray-500 mb-10">Masukkan nomor WhatsApp aktif Anda untuk memudahkan koordinasi dengan mahasiswa.</p>
                
                <form action="#" class="max-w-md space-y-6">
                    <div class="fade-up delay-200 space-y-2">
                        <label class="text-sm font-bold text-gray-600 ml-1">Nomor WhatsApp</label>
                        <input type="text" placeholder="Contoh: 08123456789" class="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-tsu-teal outline-none">
                    </div>
                    <button type="button" onclick="saveWA()" class="fade-up delay-300 bg-tsu-teal text-white px-10 py-4 rounded-2xl font-bold hover:bg-teal-700 transition shadow-lg shadow-teal-100">Simpan Nomor</button>
                </form>
            @else
...
<script>
    function switchTab(tabName) {
        const targetId = (tabName === 'documents' || tabName === 'extra') ? 'tab-extra' : 'tab-' + tabName;
        const targetBtnId = (tabName === 'documents' || tabName === 'extra') ? 'btn-extra' : 'btn-' + tabName;
